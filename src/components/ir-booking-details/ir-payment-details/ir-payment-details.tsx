...
export class IrPaymentDetails {
  ...
  private async handleIssueReceipt(detail: IPayment) {
    ...
    if (receipt_nbr || credit_receipt_nbr) {
      this.guestDocumentPreview.emit({
        documentNumber:
          payment_type?.code === PayTypes.Payment ? receipt_nbr : [PayTypes.CreditReceipt, PayTypes.Refund].includes(payment_type?.code as any) ? credit_receipt_nbr : null,
  ...
  render() {
    ...
    return [
      <wa-card appearance="plain" class="payment-details__card">
        <ir-payment-summary
